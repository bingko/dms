...
$max_id_mysql = $rs["max_roll_id"];
$next_sync_max = $max_id_mysql+10000;
$next_sync_del = $max_id_mysql-30000;
 
/// clear 50000 record
$sql_select_max = "DELETE FROM `roll_winder` WHERE `id` >=   $next_sync_del ";
$result = mysqli_query($mysql_con,$sql_select_max) or die ("ABC");
echo $sql_select_max;
...
$conn = odbc_connect("DMS","cvt","cvt") or die("Can not connect ODBC1!!!1");
 $sql = "select * from Roll_Winder where id between   '".$next_sync_del ."'  and '".$next_sync_max."' order by id ";
  echo $sql;
 echo " <hr>";
