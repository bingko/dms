...
class logsheet_folio_report extends CI_Model {
	public function get_cutsize_log()
	{
		$this->db->where('YEAR(date)',$this->get_year());
		$this->db->where('MONTH(date)',$this->get_month());
		$this->db->where('cutter',$this->get_cutter());
		$this->db->select('SUM(output) as sum_output,SUM(total_input) as sum_input,DAY(date) as day');
