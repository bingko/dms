...
$set_date_sql = date("Y-m-d");
//$set_date = "2014-12-26";
echo $set_date_sql;
 //exit;

 $conn = odbc_connect("DMS","cvt","cvt") or die("Can not connect ODBC1!!!1");
 $sql = "select * from Sheet_Product where  FinishTime >= '".$set_date_sql ."'  order by FinishTime " ;
 echo $sql;
 echo " <hr>";
...
$delete_today = "delete FROM `sheet_product` WHERE `FinishTime` LIKE '%".$set_date_sql."%'";
 
mysqli_query($mysql_con,$delete_today);
