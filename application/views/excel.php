...
<table width="100%" border="1">
                    ...
                    <td width="<?php echo 100/($countMonth+1)?>%"><div align="center">
                        <?php 
							...
							if($datetime2->format('m/Y')==$month_header->format('m/Y')){
									...
									echo "<a href='".site_url('home/result_detail/'.$value['Rollid2'].'/'.$datetime1->format('Y-m-01').'/'.$endDate->format('Y-m-01').'/intermediate/0')."' target='_blank'>";
		  ...
		  if($value['Grade']!=@$rollData[$key+1]['Grade']||$value['Basisweight']!=@$rollData[$key+1]['Basisweight']||$value['saleType']!=@$rollData[$key+1]['saleType']||$value['Brand']!=@$rollData[$key+1]['Brand']){?>
			...
			echo $name_Roll = $Grade.' '.$value['Basisweight'].' '.$value['saleType'].' '.$band_name; ?>
                    ...
                    <td width="<?php echo 100/($countMonth+1)?>%"><div align="center"> <strong>
                        <?php 
							if($stack['countRoll'][$numMonthSum]!=0){ // if total Count of Roll not equal 0
								$endDate->add(new DateInterval('P1M'));								
								echo "<a href='".site_url('home/result_detail/'.$value['Rollid2'].'/'.$datetime1->format('Y-m-01 00:00').'/'.$endDate->format('Y-m-01 00:00').'/intermediate/1')."' target='_blank'>";
              ...
              <td><div align="center"><strong><?php echo "<a href='".site_url('home/result_detail/'.$value['Rollid2'].'/'.$setDate['start_date'].'/'.$setDate['end_date'].'/intermediate/1')."' target='_blank'>";
            ...
            <tr style="background-color:#690; color:#FFF;">
			  ...
			  <?php 
			echo "<a class='wh_text' href='".site_url('home/result_detail/&nbsp;/'.$setDate['start_date'].'/'.$setDate['end_date'].'/intermediate/2')."' target='_blank'>";
